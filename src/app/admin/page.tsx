...
export default function AdminPage() {
  ...
  const fetchItems = useCallback(
    async (e?: string) => {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/${e || entity}`);
  ...
  const refreshRelatedLists = useCallback(async () => {
    try {
      const [
        ...
        fetch(API_ROUTES.PROCESOS),
        fetch(API_ROUTES.LOTES),
        fetch(API_ROUTES.CONSORCIOS),
        fetch(API_ROUTES.EMPRESAS),
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (pathPatchEntities.has(entity)) {
        res = await fetch(`/api/${entity}/${idToUse}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
        });
