    ...
    <div ref={refEl} className="flex flex-col gap-2">
      ...
      <div className="flex gap-2">
        <Button
          ...
          disabledTooltip={`Seleccione ${
            placeholder.split(' ')[2] ?? 'una opción'
          } y haga clic en la lista para habilitar`}
        >
          Guardar
        </Button>
...
export default function Page() {
  ...
  const createOferta = async (consorcioId: number | null, loteId: number) => {
    ...
    try {
      ...
      const res = await fetch(API_ROUTES.OFERTAS, {
        method: 'POST',
  ...
  const loadConsorcios = useCallback(async () => {
    try {
      const r = await fetch(API_ROUTES.CONSORCIOS);
  ...
  useEffect(() => {
    ...
    const fetchProceso = async () => {
      const url = `${API_ROUTES.PROCESOS}?id=${id}`;
      setLoading(true);
      setError(null);
      setProceso(null);
      try {
    ...
    (async () => {
      try {
        const [eRes, sRes, tRes, tfRes] = await Promise.all([
          fetch(API_ROUTES.ENTIDADES),
          fetch(API_ROUTES.ESTADO_PROCESO),
          fetch(API_ROUTES.TIPO_PROCESO),
          fetch(API_ROUTES.TIPO_FECHA_PROCESO),
        ]);
  ...
  const createOption = async (
    ...
    try {
      ...
      await Promise.all([
        (async () => {
          try {
            const r = await fetch(API_ROUTES.ENTIDADES);
        ...
        (async () => {
          try {
            const r = await fetch(API_ROUTES.TIPO_PROCESO);
  ...
  const createFecha = async () => {
    ...
    try {
      setSaving(true);
      const res = await fetch(API_ROUTES.FECHA_PROCESO, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...
          fecha: parsed,
          // importante is handled separately via the /mark-important endpoint
        }),
      });
      if (!res.ok) {
        const txt = await res.text().catch(() => undefined);
  ...
  const createLote = async () => {
    ...
    try {
      ...
      const res = await fetch(API_ROUTES.LOTES, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
  ...
  const deleteLote = async (id: number | string) => {
    ...
    try {
      ...
      setError(e?.message ?? String(e));
      showToast(
        'Error eliminando lote: ' + (e?.message || 'Error desconocido'),
        'error',
      );
    } finally {
      setSaving(false);
  ...
  const buildSecopNoticeUrl = (code: string | null | undefined) => {
    if (!code) return null;
    const c = String(code);
    return `https://www.secop.gov.co/CO1BusinessLine/Tendering/ContractNoticeView/Index?notice=${encodeURIComponent(
      c,
    )}`;
  };
  ...
  return (
    <main className="min-h-screen p-6 bg-white text-gray-900">
          ...
          <div className="text-right">
            {(() => {
              ...
              if (m) {
                ...
                if (days && hours) {
                  return (
                    <div className="text-lg font-semibold text-gray-800">{`Faltan ${days} día${
                      days === '1' ? '' : 's'
                    } y ${hours} hora${
                      hours === '1' ? '' : 's'
                    } para ${targetName}`}</div>
                ...
                if (hours)
                  return (
                    <div className="text-lg font-semibold text-gray-800">{`Faltan ${hours} hora${
                      hours === '1' ? '' : 's'
                    } para ${targetName}`}</div>
                  );
                if (mins)
                  return (
                    <div className="text-lg font-semibold text-gray-800">{`Faltan ${mins} minuto${
                      mins === '1' ? '' : 's'
                    } para ${targetName}`}</div>
                  );
              }
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="relative">
                      ...
                      <input
                        ...
                        inputMode="decimal"
                        className={`border px-2 py-1 rounded w-full ${
                          newLoteAttemptSave && loteErrors.create
                            ? 'border-red-600 focus:ring-red-200'
                            : 'focus:ring-indigo-200'
                        }`}
      ...
      {toast && (
        <div
          className={`fixed right-6 bottom-6 px-4 py-2 rounded shadow ${getToastClasses(
            toast.type,
          )}`}
        >
          {toast.message}
      ...
      <ConfirmModal
        ...
        message={
          pendingFechaDelete
            ? `¿Eliminar la fecha ${
                pendingFechaDelete.tipoNombre
                  ? `(${pendingFechaDelete.tipoNombre}) `
                  : ''
              }${
                pendingFechaDelete.fechaFormatted
                  ? `en ${pendingFechaDelete.fechaFormatted} `
                  : ''
      ...
      <ConfirmModal
        ...
        message={
          pendingOfertaDelete
            ? `¿Eliminar la oferta${
                pendingOfertaDelete.consorcioNombre
                  ? ` del consorcio ${pendingOfertaDelete.consorcioNombre}`
                  : ''
              }? Esta acción no se puede deshacer.`
