...
export default function ProcessTable() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        const res = await fetch(`${getApiBase()}/api/procesos`);
  ...
  if (!items || items.length === 0)
    return <div className="p-4">No hay procesos.</div>;
  ...
  return (
    <div className="w-full">
      ...
      <div
        ...
        className={`mb-4 p-4 bg-white rounded-lg shadow-md border overflow-hidden transition-all duration-300 ease-in-out ${filtersOpen ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'}`}
      >
        ...
        <tbody>
          {filteredItems.map((p) => (
            <tr key={p.id} className="odd:bg-white even:bg-gray-50">
              ...
              {(() => {
                // Si alguna fecha está marcada explícitamente como "importante", usarla para mostrar en la tabla
                let picked: {
                  fecha: string | null;
